...
if (
    isset($_POST['benrg']) && !empty($titre) && !empty($genre) && !empty($nom) && !empty($pays)
    && !empty($region) && !empty($annee) && !empty($nombre)
) {

    $sql = $pdo->prepare("insert into tb_livre(titre_livre,genre_livre,nom_auteur,pays_auteur,region_auteur,annee_sortie,nombre) values('$titre','$genre','$nom','$pays','$region','$annee','$nombre')");
    $sql->execute();
    if ($sql) {
...
if (isset($_POST['bsupp']) && !empty($id)) {
    ...
    $sql = $pdo->prepare("delete from tb_livre where id_livre='$id'");
    $sql->execute();
    if ($sql) {
...
<body>
    <div id="Bloc_page">
        ...
        <div align="center">
            <h1>ADMIN</h1>
            <a style="text-decoration: none; color:red" href="deconnexion.php"> se deconnecter</a>
            <h2>Formulaire d'enregistrement et de recherche des livres</h2>
            <form action="" method="POST">
                <fieldset>
                    <legend><b>ajouter des livres</b></legend>
                    <table>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td><input type="submit" name="brech" value="CHERCHER" class="bouton"></td>
                        </tr>

                        <tr>
                            <td><b>Région auteur</b></td>
                            <td><select name="region" id="region">
                                    <option value=""></option>
                                    <option value="AFRIQUE AUSTRALE">AFRIQUE </option>
                        ...
                        <tr>
                            <td><b>Nombre exemplaires</b></td>
                            <td><input type="number" min="1" name="nombre" value=""></td>
                        </tr>
                        <tr>

                            <td colspan="2"><input type="submit" name="benrg" value="ENREGISTRER" class="bouton"></td>
                        </tr>
                        <tr>
                            <td><input type="number" min="1" name="id" placeholder="Identifiant" value="<?php print $id; ?>"></td>
                            <td colspan="2"><input type="submit" name="bsupp" value="SUPPRIMER" class="bouton"></td>
                        </tr>

                        <tr>
                            <td></td>
                            <td><?php print $mess1; ?></td>
                        </tr>
                    </table>
                </fieldset>
            </form>
            ...
            if (isset($_POST['brech'])) {
                ...
                if (!empty($region)) {
                    ...
                    $rq1 = $pdo->prepare("select * from tb_livre where region_auteur like '%$region%'");
                    $rq1->execute();
                }
                ...
                if (!empty($genre) && !empty($pays)) {
                    ...
                    $rq1 = $pdo->prepare("select * from tb_livre where genre_livre like '%$genre%' and pays_auteur like '%$pays%'");
                    $rq1->execute();
                }
                //recherche par genre et région
                if (!empty($genre) && !empty($region)) {

                    $rq1 = $pdo->prepare("select * from tb_livre where genre_livre like '%$genre%' and region_auteur like '%$region%'");
                    $rq1->execute();
                }
                ...
                if (!empty($genre) && !empty($region) && !empty($annee)) {
                    ...
                    $rq1 = $pdo->prepare("select * from tb_livre where genre_livre like '%$genre%' and annee_sortie  like '%$annee%' and region_auteur like '%$region%' ");
                    $rq1->execute();
                }
