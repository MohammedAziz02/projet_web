<?php
   ...
   if(isset($valider)){
      ...
      elseif($pass!=$repass) {$erreur="Mots de passe non identiques!"; @$repass;}
         ...
         $sel=$pdo->prepare("select id from tabetudiant where login=? limit 1");
         $sel->execute(array($login));
         $tab=$sel->fetchAll();
         if(count($tab)>0)
            $erreur="Login existe déjà!";
         else{
            $ins=$pdo->prepare("insert into tabetudiant(nom,prenom,login,pass) values(?,?,?,?)");
    ...
    <?php  
        ...
        <div class="forum">
          <div class="form-info-genral">
            <div class="erreur"  style="background-color:re; color= white;"><?php echo $erreur ?></div>
            ...
            <!-- <input type="button" name="envoyer" value="Envoyer" class="buton"> -->
